...
namespace DotNet.ProjectModel.Workspaces
{
    public class ProjectJsonWorkspace : RoslynWorkspace
    {
        ...
        private Dictionary<string, ProjectId> _projectCache = new Dictionary<string, ProjectId>();

        private ProjectId AddProject(ProjectContext project)
        {
            var projectKey = project.ProjectFile.Name;
            if (_projectCache.ContainsKey(projectKey))
            {
                return _projectCache[projectKey];
            }

            var projectId = ProjectId.CreateNewId();
            _projectCache.Add(projectKey, projectId);
        ...
        private void AddSourceFile(ProjectInfo projectInfo, string file, Stream stream)
        {
            ...
            {
                folders = Path.GetDirectoryName(file).Replace(projectDir, "")
                    .Split(new[] { Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar }, StringSplitOptions.RemoveEmptyEntries);
            }
            else if (file.StartsWith(solutionDir))
            {
