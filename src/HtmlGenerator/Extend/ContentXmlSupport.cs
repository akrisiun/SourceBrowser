...
namespace Microsoft.SourceBrowser.HtmlGenerator.Extend
{
    ...
    public class ContentXmlSupport : XmlSupport, IExtend
    {
        ...
        public ContentXmlSupport(ProjectGenerator generator) : base(generator)
        {
        ...
        public void ParseProjectExtensionsArray(Project msbuildProject, string[] extension)
        {
            ...
            foreach (ProjectItem none in msbuildProject.GetItems("None"))
                if (EndsWith(none.EvaluatedInclude, extension, StringComparison.InvariantCultureIgnoreCase))
                    items.Add(none);
            foreach (ProjectItem content in msbuildProject.GetItems("Content"))
                if (EndsWith(content.EvaluatedInclude, extension, StringComparison.InvariantCultureIgnoreCase))
                    items.Add(content);
        ...
        private bool EndsWith(string item, string[] extensions, StringComparison comp = StringComparison.InvariantCultureIgnoreCase)
        {
            foreach (var check in extensions)
            {
                if (item.EndsWith(check, comp))
                    return true;
            }
        ...
        protected // override 
            string GetDisplayName() { return FilePath; }
