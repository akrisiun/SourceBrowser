    ...
    public sealed class ResourceDescription : Cci.IFileReference
    {
        ...
        internal ResourceDescription(string resourceName, string fileName, Func<Stream> dataProvider, bool isPublic, bool isEmbedded, bool checkArgs)
        {
            if (checkArgs)
            {
                ...
                {
                    throw new ArgumentNullException(name.of(dataProvider));
                }

                if (resourceName == null)
                {
                    throw new ArgumentNullException(name.of(resourceName));
                }

                if (!MetadataHelpers.IsValidMetadataIdentifier(resourceName))
                {
                    throw new ArgumentException(CodeAnalysisResources.EmptyOrInvalidResourceName, name.of(resourceName));
                }
                ...
                if (!isEmbedded)
                {
                    if (fileName == null)
                    {
                        throw new ArgumentNullException(name.of(fileName));
                    }

                    if (!MetadataHelpers.IsValidMetadataFileName(fileName))
                    {
                        throw new ArgumentException(CodeAnalysisResources.EmptyOrInvalidFileName, name.of(fileName));
                    }
                }
            }
