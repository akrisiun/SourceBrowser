...
namespace Microsoft.CodeAnalysis
{
    ...
    public struct MetadataReferenceProperties : IEquatable<MetadataReferenceProperties>
    {
        ...
        public MetadataReferenceProperties(MetadataImageKind kind = MetadataImageKind.Assembly, ImmutableArray<string> aliases = default(ImmutableArray<string>), bool embedInteropTypes = false)
        {
            if (!kind.IsValid())
            {
                throw new ArgumentOutOfRangeException(name.of(kind));
            }
            ...
            if (kind == MetadataImageKind.Module)
            {
                ...
                {
                    throw new ArgumentException(CodeAnalysisResources.CannotEmbedInteropTypesFromModule, name.of(embedInteropTypes));
                }

                if (!aliases.IsDefaultOrEmpty)
                {
                    throw new ArgumentException(CodeAnalysisResources.CannotAliasModule, name.of(aliases));
                }
            }
            ...
            if (!aliases.IsDefaultOrEmpty)
            {
                foreach (var alias in aliases)
                {
                    ...
                    {
                        throw new ArgumentException(CodeAnalysisResources.InvalidAlias, name.of(aliases));
                    }
                }
            }
