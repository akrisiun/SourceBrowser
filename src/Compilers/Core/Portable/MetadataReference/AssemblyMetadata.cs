...
namespace Microsoft.CodeAnalysis
{
    ...
    public sealed class AssemblyMetadata : Metadata
    {
        ...
        public static AssemblyMetadata Create(ModuleMetadata module)
        {
            if (module == null)
            {
                throw new ArgumentNullException(name.of(module));
            }
        ...
        public static AssemblyMetadata Create(ImmutableArray<ModuleMetadata> modules)
        {
            ...
            {
                throw new ArgumentException(name.of(modules));
            }

            if (modules.Length == 0)
            {
                throw new ArgumentException(CodeAnalysisResources.AssemblyMustHaveAtLeastOneModule, name.of(modules));
            }
            ...
            for (int i = 0; i < modules.Length; i++)
            {
                if (modules[i] == null)
                {
                    throw new ArgumentNullException(name.of(modules) + "[" + i + "]");
                }

                if (!modules[i].IsImageOwner)
                {
                    throw new ArgumentException(CodeAnalysisResources.ModuleCopyCannotBeUsedToCreateAssemblyMetadata, name.of(modules) + "[" + i + "]");
                }
            }
        ...
        public ImmutableArray<ModuleMetadata> GetModules()
        {
            ...
            {
                throw new ObjectDisposedException(name.of(AssemblyMetadata));
            }
