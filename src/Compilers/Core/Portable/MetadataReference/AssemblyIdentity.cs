...
namespace Microsoft.CodeAnalysis
{
    ...
    public sealed partial class AssemblyIdentity : IEquatable<AssemblyIdentity>
    {
            ...
            AssemblyContentType contentType = AssemblyContentType.Default)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(name.of(contentType), CodeAnalysisResources.InvalidContentType);
            }

            if (!IsValidName(name))
            {
                throw new ArgumentException(string.Format(CodeAnalysisResources.InvalidAssemblyName, name), name.of(name));
            }

            if (!IsValidCultureName(cultureName))
            {
                throw new ArgumentException(string.Format(CodeAnalysisResources.InvalidCultureName, cultureName), name.of(cultureName));
            }

            // Version allows more values then can be encoded in metadata:
            if (!IsValid(version))
            {
                throw new ArgumentOutOfRangeException(name.of(version));
            }
            ...
            if (hasPublicKey)
            {
                ...
                {
                    throw new ArgumentException(CodeAnalysisResources.InvalidPublicKey, name.of(publicKeyOrToken));
                }
            }
            else
            {
                ...
                {
                    throw new ArgumentException(CodeAnalysisResources.InvalidSizeOfPublicKeyToken, name.of(publicKeyOrToken));
                }
            }

            if (isRetargetable && contentType == AssemblyContentType.WindowsRuntime)
            {
                throw new ArgumentException(CodeAnalysisResources.WinRTIdentityCantBeRetargetable, name.of(isRetargetable));
            }
        ...
        public static AssemblyIdentity FromAssemblyDefinition(Assembly assembly)
        {
            if (assembly == null)
            {
                throw new ArgumentNullException(name.of(assembly));
            }
