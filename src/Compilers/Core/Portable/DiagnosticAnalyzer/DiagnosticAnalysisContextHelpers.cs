...
namespace Microsoft.CodeAnalysis.Diagnostics
{
    internal static class DiagnosticAnalysisContextHelpers
    {
        ...
        internal static void VerifyArguments(Diagnostic diagnostic, Compilation compilationOpt, Func<Diagnostic, bool> isSupportedDiagnostic)
        {
            ...
            if (diagnostic == null)
            {
                throw new ArgumentNullException(name.of(diagnostic));
            }
            ...
            {
                throw new ArgumentException(string.Format(CodeAnalysisResources.UnsupportedDiagnosticReported, diagnostic.Id), name.of(diagnostic));
            }
            ...
            if (!UnicodeCharacterUtilities.IsValidIdentifier(diagnostic.Id))
            {
                ...
                throw new ArgumentException(string.Format(CodeAnalysisResources.InvalidDiagnosticIdReported, diagnostic.Id), name.of(diagnostic));
            }
        }
        ...
        private static void VerifyAction<TContext>(Action<TContext> action)
        {
            if (action == null)
            {
                throw new ArgumentNullException(name.of(action));
            }
        }
        ...
        private static void VerifySymbolKinds(ImmutableArray<SymbolKind> symbolKinds)
        {
            ...
            {
                throw new ArgumentNullException(name.of(symbolKinds));
            }

            if (symbolKinds.IsEmpty)
            {
                throw new ArgumentException(CodeAnalysisResources.ArgumentCannotBeEmpty, name.of(symbolKinds));
            }
        }
            ...
            where TLanguageKindEnum : struct
        {
            ...
            {
                throw new ArgumentNullException(name.of(syntaxKinds));
            }

            if (syntaxKinds.IsEmpty)
            {
                throw new ArgumentException(CodeAnalysisResources.ArgumentCannotBeEmpty, name.of(syntaxKinds));
            }
        }
            ...
            where TKey: class
        {
            if (key == null)
            {
                throw new ArgumentNullException(name.of(key));
            }

            if (valueProvider == null)
            {
                throw new ArgumentNullException(name.of(valueProvider));
            }
        }
    }
}
