...
namespace Microsoft.CodeAnalysis.Diagnostics
{
    ...
    internal struct AnalysisContextInfo
    {
        ...
        public string GetContext()
        {
            ...
            {
                sb.AppendLine(string.Format("{0} : {1}", name.of("Compilation"), _compilation.AssemblyName));
            }

            if (_operation != null)
            {
                sb.AppendLine(string.Format("{0} : {1}", name.of("IOperation"), _operation.Kind));
                }

            if (_symbol?.Name != null)
            {
                sb.AppendLine(string.Format("{0} : {1}", name.of("ISymbol"), _symbol.Name, _symbol.Kind));
            }

            if (_tree?.FilePath != null)
            {
                sb.AppendLine(string.Format("{0} : {1}", name.of("SyntaxTree"), _tree.FilePath));
            }
            ...
            if (_node != null)
            {
                ...
                sb.AppendLine(string.Format(string.Format("{0} : {1}",
                        name.of("SyntaxNode"), GetFlattenedNodeText(_node)) 
                        ));
                        //} [{_node.GetType().Name}]@{_node.Span.ToString()} {(lineSpan.HasValue ? lineSpan.Value.ToString() : string.Empty));
            }
