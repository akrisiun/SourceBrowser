...
namespace Microsoft.CodeAnalysis
{
    ...
    public class SourceFileResolver : SourceReferenceResolver, IEquatable<SourceFileResolver>
    {
            ...
            ImmutableArray<KeyValuePair<string, string>> pathMap)
        {
            ...
            {
                throw new ArgumentNullException(name.of(searchPaths));
            }

            if (baseDirectory != null && PathUtilities.GetPathKind(baseDirectory) != PathKind.Absolute)
            {
                throw new ArgumentException(CodeAnalysisResources.AbsolutePathExpected, name.of(baseDirectory));
            }
            ...
            if (!pathMap.IsDefaultOrEmpty)
            {
                foreach (var kv in pathMap)
                {
                    ...
                    {
                        throw new ArgumentException(CodeAnalysisResources.EmptyKeyInPathMap, name.of(pathMap));
                    }

                    var value = kv.Value;
                    if (value == null)
                    {
                        throw new ArgumentException(CodeAnalysisResources.NullValueInPathMap, name.of(pathMap));
                    }

                    if (IsPathSeparator(key[key.Length - 1]))
                    {
                        throw new ArgumentException(CodeAnalysisResources.KeyInPathMapEndsWithSeparator, name.of(pathMap));
                    }
                }
            }
        }
        ...
        {
            CompilerPathUtilities.RequireAbsolutePath(resolvedPath, name.of(resolvedPath));
