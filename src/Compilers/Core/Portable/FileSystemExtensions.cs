...
namespace Microsoft.CodeAnalysis
{
    public static class FileSystemExtensions
    {
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (var outputStream = FileUtilities.CreateFileStreamChecked(PortableShim.File.Create, outputPath, name.of(outputPath)))
            using (var pdbStream = (pdbPath == null ? null : FileUtilities.CreateFileStreamChecked(PortableShim.File.Create, pdbPath, name.of(pdbPath))))
            using (var xmlDocStream = (xmlDocPath == null ? null : FileUtilities.CreateFileStreamChecked(PortableShim.File.Create, xmlDocPath, name.of(xmlDocPath))))
            using (var win32ResourcesStream = (win32ResourcesPath == null ? null : FileUtilities.CreateFileStreamChecked(PortableShim.File.OpenRead, win32ResourcesPath, name.of(win32ResourcesPath))))
            {
