...
namespace Microsoft.CodeAnalysis.Text
{
    ...
    public struct TextSpan : IEquatable<TextSpan>, IComparable<TextSpan>
    {
        ...
        public static TextSpan FromBounds(int start, int end)
        {
            if (start < 0)
            {
                throw new ArgumentOutOfRangeException(name.of(start), CodeAnalysisResources.StartMustNotBeNegative);
            }

            if (end < start)
            {
                throw new ArgumentOutOfRangeException(name.of(end), CodeAnalysisResources.EndMustNotBeLessThanStart);
            }
