...
//  http://localhost:5004/index.html#System.Web/Compilation/BuildManager.cs,fb803c621f3806a8
function redirectLocation(frame, newLocation) {
    ...
    if (newLocation.indexOf(",index") > 0)
    {
        alert(",index=" + newLocation);
...
function processHash() {
    ...
    if (anchor) {
        ...
        {
            if (potentialFile == "index.html")
                redirectLocation(n, folder + "/ProjectExplorer.html");
            else 
...
function rewriteExternalLink(link) {
    ...
    if (firstIndex > -1) {
        ...
        externalUrl = "/";  //index.html#";
        url = externalUrl + url.slice(indexLength + 1);
        
        externalUrl = "/index.html";
        if (url.substr(0, 8) == "#System.")
            url = externalUrl + url;
